...
| D32 | **High — OPEN** | **`guardlink link --remove` truncates agent files at the workspace marker.** `cleanupRemovedRepo` computes `endIdx` — the end of the workspace block, exactly as its own comment intends ("Remove from marker to next ## or end of file") — and then ignores it: the write is `content.slice(0, markerIdx).trimEnd() + '\n'`, which discards everything from the marker to EOF. **Any section a user wrote after the workspace block is destroyed.** Reproduced: a file with `## Workspace Context` followed by `## My Own Section` comes back as just the header, user content gone. Same family as D24 — a command losing work the user did not ask it to touch. The correct write is `+ '\n' + content.slice(endIdx)`. **Not fixed here**: it is a behaviour change needing its own test, and this was a lint cleanup. Found by the no-unused-vars cleanup | `src/workspace/link.ts:300-304`; reproduced 2026-08-10 |
| D33 | **Med — OPEN** | **The MCP server ships actively false guidance.** `src/mcp/instructions.ts:49` tells every connected agent that a `.gal` under `test/` is "silently dropped — do not put one there until GL-503 lands." GL-503 landed in `4b49ece`; D4 is verified fixed. The instructions arrive at initialize, before any tool call, so this is the *first* thing an agent reads — steering it away from a directory that now works. One-line fix. Root cause is structural: `instructions.ts` hand-writes claims about parser behaviour with nothing tying the two together, the same class as D19's hand-written grammar examples | verified: the string is still present on `HEAD` |
