...
> **Status: 35 logged · 31 fixed · 1 won't-fix (D20) · 3 open (D22, D34, D35).**
>
...
| D34 | **High — OPEN** | **`guardlink_graph` returns the entire `unannotated_files` list.** `selectSubgraph` returns `{...model, <filtered arrays>}`; the spread preserves `annotated_files` and `unannotated_files` untouched while every relation array is filtered. Measured on specter-v1: **8,094 rows = 631 KB = 100% of the subgraph payload**, for a query whose traversal resolved nothing and returned zero nodes. GL-205 measured this exact problem for `guardlink_parse` and dropped the field by default; `guardlink_graph` shipped two commits later and reintroduced it via the spread. Invisible on this repo (23 rows, 1%), which is why five phases never surfaced it. Fix: filter or omit both file lists in `selectSubgraph`, matching GL-205's ruling | measured over real stdio on specter-v1: 704 KB response, 0 nodes |
| D35 | Low — OPEN | **The `guardlink-mcp` bin is a no-op.** `package.json` declares it as `./dist/mcp/index.js`, but that module *exports* `startStdioServer` and never calls it — no shebang, no main guard. Executing it loads the module and exits. Low impact because `init` writes `guardlink mcp` (the CLI subcommand, which works) into `.mcp.json`, so no generated config points at the dead entry — but it is an advertised entry point that does nothing | verified: piping an initialize request to it produces no response |
