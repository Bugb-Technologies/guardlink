...
| D26 | Med | *(found in Phase 4 review)* **`.guardlink/model.json` carries a top-level `generated_at`.** The `.mmd` headers had volatile fields stripped in `a921afa`; `model.json` did not. It is the artifact GL-304 most deliberately commits — justified precisely so a model change shows up in PR review — and with the pre-commit hook regenerating on every commit it now carries a spurious one-line diff every time, training reviewers to ignore diffs on the one file whose diffs were meant to matter | verified: two `guardlink artifacts .` runs differ only in `generated_at` |
