...
Current model: No annotations parsed yet.
...
| Path | What it is |
|---|---|
| `definitions.py` | **Definitions.** Every `@asset`, `@threat` and `@control`, each with a `#id`. Read this first — it is the vocabulary everything else references. |
...
| `annotations/` | The annotations themselves, as `.gal` sidecars mirroring source paths. |
...
**external** — annotations live in `.guardlink/annotations/`, not in source files.
...
**Definitions go in `definitions.py`, always — in both modes.** Reuse existing `#id`s; never
...
**Definitions** — only in `definitions.py`:
...
**The complete reference is `.guardlink/GUARDLINK_REFERENCE.md`** — every verb, every alias, the conformance
