 ...
 *
 * @exposes #cli to #path-traversal [high] cwe:CWE-22 -- "[internal] User-supplied dir argument resolved via path.resolve; local dev controls CLI args"
 * @mitigates #cli against #path-traversal using #path-validation -- "resolve() canonicalizes paths; cwd-relative by design"
 * @exposes #cli to #arbitrary-write [high] cwe:CWE-73 -- "[internal] init/report/sarif/dashboard write files to user-specified paths; local dev controls output directories"
 * @mitigates #cli against #arbitrary-write using #path-validation -- "Output paths resolved relative to project root"
 * @exposes #cli to #api-key-exposure [high] cwe:CWE-798 -- "[internal] API keys handled in config set/show commands; local dev manages their own keys"
 * @mitigates #cli against #api-key-exposure using #key-redaction -- "maskKey() redacts keys in show output"
 * @exposes #cli to #cmd-injection [critical] cwe:CWE-78 -- "[internal] Agent launcher spawns child processes; local dev triggers annotate/threat-report commands"
...
3. Your final response MUST be the COMPLETE, FULL ${reportLabel} — every section, every finding, in full detail. Do NOT write a summary, orientation, or recap of the report; write the actual report itself.
4. Be specific — reference actual files, functions, and line numbers from the codebase
5. Do NOT write phrases like "the report has been delivered above", "here's a quick orientation", or "as shown earlier" — if content belongs in the report, write it directly in your response
6. Output ONLY the markdown report content — do NOT add metadata comments, save confirmations, or file path messages
7. Do NOT include lines like "Generated by...", "Agent:", "Project:", or "The report file write was blocked..."`;
