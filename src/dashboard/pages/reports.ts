 ...
 */
import { esc, scopeLabel, sectionHead, copyButton, icon } from '../html.js';
...
<div id="sec-ai-analysis" class="section-content">
  ${sectionHead(icon('file'), 'Threat Reports', null, `<span class="muted">${analyses.length} saved</span>`)}
${scope ? `  <p class="scope-note">${icon('alert')} These reports are <strong>whole-project</strong> documents. Unlike the rest of this page they are not narrowed to ${esc(scopeLabel(scope))}, and they may discuss assets outside it.</p>` : ''}
  ...
  <div class="panel ai-analysis-panel">
    <div class="report-toolbar">
      ...
      <button class="btn btn-primary" data-copy-report title="Copy the whole report as markdown">${icon('copy')} Copy report</button>
      <button class="btn" data-download-report title="Save the report as a .md file">${icon('download')} Download .md</button>
    </div>
