...
const TOOLS = `
            ...
            <button class="diagram-btn" data-copy-diagram title="Copy the Mermaid source — paste it into mermaid.live or a markdown file">⧉ Source</button>`;
...
export function renderDiagramsPage(ctx: PageContext): string {
  ...
  if (threatGraph) {
    ...
    panels.push(shell('threat-graph', 'Threat Graph',
      `
        ...
        `,
      `Assets, threats, controls, and mitigations. ${hasFullVariant ? 'Filtered to high/critical by default — click <em>All severities</em> to expand.' : ''}`,
      hasFullVariant ? `
  ...
  if (dataFlow) {
    ...
    panels.push(shell('data-flow', 'Data Flow', `<pre class="mermaid">\n${esc(dataFlow)}\n</pre>`,
      'Trust zones (🧱) and data movement across system boundaries. Each boundary shows both sides of the trust line.', '', panels.length === 0));
  }
  if (attackSurface) {
    tabs.push({ id: 'attack-surface', label: 'Attack Surface', icon: '⚠' });
    panels.push(shell('attack-surface', 'Attack Surface', `<pre class="mermaid">\n${esc(attackSurface)}\n</pre>`,
      'Exposures per asset, severity-coloured. <strong>💥 confirmed</strong>, <strong>⚠️ open</strong>, <strong>✅ mitigated</strong>, <strong>🟦 accepted</strong>.', '', panels.length === 0));
  }
