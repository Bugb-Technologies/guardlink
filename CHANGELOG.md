...
- **Test coverage** — added `tests/lookup.test.ts` (14 tests across the MCP query DSL, including regression guards for the working queries and failing-then-fixed cases for the resolver bugs above) and `tests/pentest-loader.test.ts` (10 tests covering JSON, Python, and YAML conventions for template metadata extraction). Suite total: 72 → 96.
