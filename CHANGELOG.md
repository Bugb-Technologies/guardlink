...
- **Playbooks, the evidence bar and the gate.** `guardlink annotate` used to leave the method to whoever typed the prompt. Now the tool owns it: a playbook supplies the phases, the evidence bar and the stop conditions, the prompt supplies scope and intent, and the same words always pick the same method.
  - `--playbook map|exploitable|chains|diff|coverage|verify` (inferred from the prompt by rule when omitted, `exploitable` by default, the choice announced on stderr). Every playbook carries the bar: an `@exposes` names entry point, input, sink and the absent control, or it is written as `@audit`; severity stays within the threat's declared band; `@confirmed` needs evidence; an agent never writes `@accepts` or `@entitles`.
  - **The gate.** After a terminal agent returns, the run's additions are found by claim key and linted against the bar. Violations go back to the agent (`--gate-retries`, default 1); what still fails is removed from the tree, printed, and the command exits 1. `--no-gate` skips it.
  - **`guardlink lint [dir] [--since <ref>] [--json]`** runs the same check standalone: the whole tree, or only what was added since a ref. Exit 1 on any error. Output `guardlink.lint/v1`.
  - **Threat reports carry findings a machine can read.** Every framework prompt ends with a required `guardlink-findings` JSON block; it is parsed after the run, its asset and threat ids are validated against the model, and the dashboard renders it as a table of linked rows above the prose. `--shape full|executive|pr|audit` chooses the audience; free text is now a focus under the framework, never a replacement for it.
  - **`guardlink init`** writes each playbook as a Claude Code skill under `.claude/skills/`; a skill file without the generated marker is authored and kept. `guardlink_annotate` (MCP) takes `playbook`.
  - Library: `selectAnnotatePlaybook`, `getPlaybook`, `skillFileFor` from `src/playbooks`; `lintAnnotations`, `runGate`, `buildGateFollowUp`, `stripViolations` from `src/gate`; `parseFindingsBlock`, `validateFindings`, `renderFindingsTable` from `guardlink/analyze`.
