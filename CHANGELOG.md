...
- **A scan result can no longer be recorded against a claim the probe never tested.** `guardlink sarif` stamps each result with `guardlink/threatId`, derived from `(asset, threat, file)` with no line in it. Two `@exposes` in one file naming the same asset and the same threat therefore mint **one** id. Delete the first and let the second come to sit on the line the first was tested at, and the surviving claim matches every stamped value there is — file, line, asset, threat, id — so `guardlink hypothesis confirm --from-scan` joined the finding to it and wrote a confirmation, with nothing anywhere looking wrong. No care taken by the consumer could detect this: the discriminator did not exist to send.
  ...
  - **Each result now carries its claim key**, as a second SARIF-native keyed fingerprint `guardlink/claimKey`, mirrored to `properties.claimKey` exactly as `threatId` already is. The key was already there — it is the one the hypothesis ledger keys an entry on (`src/parser/claim-key.ts`) — and it is a digest of the claim's **own words**: verb, identity arguments, external refs, description, plus the file. Every claim has one, so unlike a code hash there is no absent case to omit. The join wanted claim identity, not code identity, and this is the identity the ledger already uses to say which claim an outcome belongs to.
  - **`--from-scan` treats a mismatch as a non-match.** A finding carrying `claim_key` joins only to the claim whose key is that key; the rest are reported as **stale** — the claim this was tested against is not in the model any more, deleted or had its asset, threat, refs, description or file edited — naming the claims it was refused against and the command to record it by hand. Nothing is written for a stale finding and the command exits non-zero. A report with no stamp joins exactly as it did before.
  - **It also resolves joins that were previously ambiguous.** A key matches at most one claim, so a finding that falls through to the asset-and-threat join and fits both siblings resolves to the one the stamp names instead of being handed back.
